...
import { ansibleSettingsTranslations } from "@/lang/ansible-settings";
...
export default function AnsibleSettingsPage() {
  ...
  const t = ansibleSettingsTranslations[language];
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        ...
        const configResponse = await fetch("/api/auth/awx-config", { cache: "no-store" });
        const configPayload = (await configResponse.json()) as AwxConfigResponse;
        ...
        if (configPayload.config) {
          setBaseUrl(configPayload.config.baseUrl?.trim() ?? "");
          setApiToken(configPayload.config.apiToken ?? "");
        }
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch("/api/auth/awx-config", {
        method: "PUT",
  ...
  const handleTest = async () => {
    ...
    try {
      const response = await fetch("/api/auth/awx-config/test", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ baseUrl, apiToken }),
      });

      const payload = (await response.json()) as AwxTestResponse;
      if (!response.ok || !payload.ok) {
        ...
        <section className={panelClass}>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            <label className="text-xs sm:text-sm">
              <span className={`mb-1 block ${topLabelClass}`}>{t.hostLabel}</span>
              <input value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} className={inputClass} />
            </label>
            <label className="text-xs sm:text-sm">
              <span className={`mb-1 block ${topLabelClass}`}>{t.tokenLabel}</span>
              <input type="password" value={apiToken} onChange={(event) => setApiToken(event.target.value)} className={inputClass} />
            </label>
          </div>
